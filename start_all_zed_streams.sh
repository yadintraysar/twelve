...
echo "Cameras: 0 (port 5001), 1 (port 5002), 2 (port 5004) - Camera 3 DISABLED"
echo ""

# Set display for GUI applications
export DISPLAY=:0

# Camera ID 0 - Port 5001 (has issues but will attempt)
...
echo ""
echo "Cameras 0, 1, 2 started successfully! (Camera 3 disabled for now)"
echo "Process IDs:"
echo "  Camera 0: $PID_CAM0"
...
echo ""
echo "To stop all streams, run: kill $PID_CAM0 $PID_CAM1 $PID_CAM2"
echo "Or use: pkill -f 'gst-launch.*zedsrc'"
echo ""
echo "Note: Camera 0 may show corrupted frame warnings but should continue streaming"
echo "      thanks to the modified error handling in the ZED GStreamer plugin."
