...
void GLViewer::draw() {
    ...
    // Draw camera overlay first (in background)
    drawCameraOverlay();
...
void GLViewer::drawCameraOverlay() {
    std::lock_guard<std::mutex> lock(camTexMtx);
    if (!camTextureInitialized) return;

    // Set up simple textured quad covering lower-right corner
    ...
    // Size: quarter of window, aspect preserved
    float quadW = w * 0.35f;
    float quadH = quadW * ((float)camTexHeight / (float)camTexWidth);
    float x0 = w - quadW - 20.0f;
    float y0 = 20.0f;
    float x1 = x0 + quadW;
    float y1 = y0 + quadH;
...
void GLViewer::updateCameraImage(const sl::Mat& image) {
    std::lock_guard<std::mutex> lock(camTexMtx);
    ...
    // If resolution changes, recreate
    ...
    // Upload current frame (assumes image is RGBA on CPU)
    glBindTexture(GL_TEXTURE_2D, camTextureId);
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, image.getPtr<sl::uchar1>());
    glBindTexture(GL_TEXTURE_2D, 0);
}
