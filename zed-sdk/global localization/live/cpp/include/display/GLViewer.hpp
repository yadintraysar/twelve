...
public:
    ...
    // Update the viewer with the latest camera image (RGBA, CPU memory)
    void updateCameraImage(const sl::Mat& image);
...
private:
    ...
    // Camera image overlay texture
    bool camTextureInitialized = false;
    GLuint camTextureId = 0;
    int camTexWidth = 0;
    int camTexHeight = 0;
    std::mutex camTexMtx;
};
